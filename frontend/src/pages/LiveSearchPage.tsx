...
export default function LiveSearchPage() {
  const [selectedPlatforms, setSelectedPlatforms] = useState<Set<string>>(new Set(["bnn", "jib"]));
      ...
      <div className="card p-4 mb-4">
        ...
        <div className="flex flex-wrap gap-2">
          {PLATFORMS.map((p) => {
            ...
            // Lazada requires a signed session token generated by its JS —
            // not available in headless/cloud mode.
            const isCloudLimited = p.id === "lazada";
            return (
              <button key={p.id} onClick={() => togglePlatform(p.id)} disabled={isSearching}
                title={isCloudLimited ? "Lazada ต้องการ session token — อาจไม่มีผลลัพธ์บน cloud" : undefined}
          ...
          <p className="w-full text-[10px] text-gray-400 mt-1">
            * Lazada ต้องใช้ session token — อาจไม่มีผลลัพธ์เมื่อค้นหาจาก cloud
          </p>
        </div>
      </div>
